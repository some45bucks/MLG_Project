...
from modified_sample_factory.algo.utils.action_distributions import is_continuous_action_space, sample_actions_log_probs
from modified_sample_factory.algo.utils.running_mean_std import RunningMeanStdInPlace, running_mean_std_summaries
...
from modified_sample_factory.model.action_parameterization import (
    ActionParameterizationContinuousNonAdaptiveStddev,
    ActionParameterizationDefault,
)
from modified_sample_factory.model.model_utils import model_device
from modified_sample_factory.utils.normalize import ObservationNormalizer
from modified_sample_factory.utils.typing import ActionSpace, Config, ObsSpace
...
class ActorCriticGeo(ActorCriticSharedWeights):
    def __init__(self,
                model_factory,
                obs_space: ObsSpace,
                action_space: ActionSpace,
...
def default_make_actor_critic_geo_func(cfg: Config, obs_space: ObsSpace, action_space: ActionSpace,metadata) -> ActorCritic:
    from modified_sample_factory.algo.utils.context import global_model_factory

    model_factory = global_model_factory()

    return ActorCriticGeo(model_factory, obs_space, action_space,metadata, cfg)

def create_actor_critic_geo(cfg: Config, obs_space: ObsSpace, action_space: ActionSpace, metadata) -> ActorCriticGeo:
    # check if user specified custom actor/critic creation function
    from modified_sample_factory.algo.utils.context import global_model_factory

    make_actor_critic_geo_func = global_model_factory().make_actor_critic_geo_func
    return make_actor_critic_geo_func(cfg, obs_space, action_space,metadata)
