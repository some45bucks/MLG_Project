...
from modified_sample_factory.algo.utils.multiprocessing_utils import get_lock, get_mp_ctx
...
class ParameterClientAsync(ParameterClient):
    ...
    def _init_local_copy(self, device, cfg, obs_space, action_space, metadata):
        if cfg.use_geo:
            self._actor_critic = create_actor_critic_geo(cfg, obs_space, action_space,metadata)
        else:
    ...
    def on_weights_initialized(self, state_dict, device: torch.device, policy_version: int) -> None:
        ...
        self._init_local_copy(device, self.cfg, self.env_info.obs_space, self.env_info.action_space, self.env_info.metadata)
